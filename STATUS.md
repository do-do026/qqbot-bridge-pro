...
| 完整设置 UI（G6） | ✅ | ✅ | ✅ | 2026-08-16 compose_dsl 设置页完成（screen 传占位符 + LazyColumn 滚动） |
...
| **G6 UI** | 完整设置界面（compose_dsl 设置页：状态/凭证/自动化/群聊/控制） | ✅ **完成**（2026-08-16：入口 screen 修复 + LazyColumn 滚动 + 文案对齐 schema，真机通过） |
...
|8. ~~真机验证引用气泡~~ ✅ 2026-08-16 已确认：**群聊机器人被动回复场景下，QQ 客户端不渲染引用卡片**（`message_reference` 已正确发出、平台返回 `ref_idx`，但客户端按 `msg_id` 被动回复渲染，不显示引用卡片）——平台限制，非 bug
...
- **G6 设置 UI（2026-08-16）**：compose_dsl 设置页（状态/凭证/自动化/群聊/控制），screen 占位符修复 + LazyColumn 滚动，真机通过
- **G5 落盘验证（2026-08-16）**：已确认 automatic 上下文随 `persist_turn` 落盘进 Operit 历史（v1.11.0/v1.12.1/main 源码三版一致）；决策：接受落盘 + 文档标注，未来可用 Prompt Hook 隔离（hook 风险已核实：异常隔离 + 超时双防护，不会崩宿主）
...
- **G3 引用气泡**：`message_reference` 已正确发出、平台返回 `ref_idx`，但群聊机器人被动回复场景下 QQ 客户端不渲染引用卡片（客户端按 `msg_id` 被动回复渲染）。保留 `message_reference` 无害（频道/未来版本可能生效），不强行去掉 `msg_id`（会失去被动回复时效保护）。
