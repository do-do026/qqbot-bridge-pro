...
> 更新时间：2026-08-07 00:3x（G1 群事件分流/可恢复缓存完成）。新窗口先读 `V2-BLUEPRINT.md` §12，再读 `STATUS.md`；G0/G1 已完成，从 Epic G4 开始实施。
...
- 未绑定 C2C 按 openid 分对话；`target_chat_id` 已废弃（2026-08-07 初尘实测确认），群消息一律按 group_openid 新建/复用独立对话。
...
- src/dist 一致、sync.sh 全过、27 项冒烟测试全过；dev_package 已同步，**未烧录**（新会话可见新工具）。
...
2. ~~**G1 事件分流/缓存**~~ ✅ 代码完成（2026-08-07）：@ 触发分流、普通群消息进持久化环形缓存、每群 firstAt、单群30/全局100双层容量、当天恢复窗口、并发 flush、群开关粒度清理、`qqbot_pro_group_context` 查询工具；22 项冒烟测试过，**未烧录未实测**。
3. **G4 统一 chunker**：`。！？\n`、连续换行归一化、400 字符兜底。（当前）
4. **G2 上下文工具收尾**：automatic 自动附带模式；三态开关接入运行路径。
5. **G3 replyTo/引用/过期降级**。
6. **G5 Prompt Finalize Hook 探针**，验证成功后再实现不落盘桥接 Prompt。
7. 可靠性 Sprint：事务幂等、token 缓存、错误码/Trace ID、故障注入。
8. **最后做 G6 UI**，禁止在旧 UI 上继续补丁堆叠。
9. 每个 Epic 完成后更新 STATUS 的代码/部署/验证三态；全部实测后再发布。

> 烧录提醒：G0 代码完成但未烧录。烧录后旧会话通常看不到新工具，需新开会话验证；烧录前确认当前工作不依赖运行中的旧版桥接。
