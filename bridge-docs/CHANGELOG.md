...
- **决策**：保留 `message_reference`（无害，频道/未来版本可能生效）；不强行去掉 `msg_id`（去掉会变主动消息，失去被动回复时效保护 + 主动消息额度）。
- **文档**：STATUS §3/§4/§6、ARCHITECTURE §7 状态 + §7.5 + §11 已更新，G3 状态从「待真机验证」改为「已确认平台限制」。
- **下一步主线**：G5 Hook 探针（验证 automatic 上下文附件是否落盘）。
