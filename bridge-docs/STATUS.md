...
| 前后文三态 | ✅ 已完成（G2 2026-08-10 实测闭环） | off/automatic/agent_on_demand 全实现；automatic 自动附带邻近上下文附件（前/后各5、最多20、G7标签），03:14 初尘实测 AI 能读到普通消息上下文 |
| 前后文条数 | ✅ 已完成（G2 2026-08-10） | 前5/后5，单次最多20；查询工具已实现默认值；automatic 附件同样 clamp |
...
- **桥接状态**：已重新启用（enabled=true）；C2C 绑定指向 604898bd（初尘新窗）；Gateway 正常收消息。
- **群链路已实测（02:1x-02:2x）**：关键词/@ 触发 → 自动建 `[QQ][群]` 对话窗 ✅；AI 回复完整 ✅（T041）；@ 识别跨 member_openid 已修（T042）；**回传 QQ 最终确认待复测**。
- **配置**：keyword_or_at + 关键词[渡渡,dodo,渡渡渡渡] + 5s 聚合窗口 + 单群缓存 30。
- **语气问题（跟进中）**：绑定对话的角色卡决定回复人格（604898bd 当前绑 flash 系角色卡）；渡渡本人语气也疑似"被夺舍"（可能涉及模型 provider / assistantInstruction / 角色卡关联），初尘正在排查，**后续再议，勿动现有绑定**。
...
3. ~~G4 统一 Waifu chunker~~ ✅ 已完成（2026-08-09，29/29 测试）。
4. **G3 replyTo 编号回复与时效降级**（当前入口；G2 已闭环：查询工具 + automatic 自动附带均完成）。
5. ~~G7 群成员身份绑定最小版~~ ✅ 已完成（2026-08-08：初尘 openid → "初尘"，聚合/查询上下文标签生效）。
6. **G7 完整版（UI 管理群成员绑定）**（2026-08-08 新需求）。
7. **G5 Prompt Finalize Hook 探针**。
8. 可靠性 Sprint。
9. G6 完整 UI。
10. 全链路验收后才更新代码/部署/验证状态。
