...
5. `target_chat_id` **已废弃（2026-08-07 初尘实测确认）**：群聊不再支持固定目标，群消息（含 @Bot）一律按 `group:{group_openid}` 新建/复用独立对话；字段仅保留兼容读取，不参与路由。
6. **落盘边界（2026-08-07 初尘确认）**：只有 @ 触发消息（连同 AI 回复）持久化进 Operit 对话历史；普通群消息只存在于桥接侧上下文缓存/Gateway 消息列表，永不进入 Operit 对话历史。
7. **时间戳口径**：落盘的 @ 消息必须携带最短时间戳（`sentAt`/`receivedAt`），避免 AI 把积压消息误判为实时；桥接控制 Prompt 与上下文查询结果只发给模型、不落盘（G5 验证）。
...
- **落盘边界（2026-08-07 初尘确认）**：仅 @ 触发消息及其 AI 回复落进 Operit 对话历史；普通群消息、上下文查询结果与桥接控制 Prompt 均不落盘。普通群消息只存在桥接侧缓存（内存 + 持久化文件），供 agent 按需查询。
...
- **持久化与恢复（2026-08-07 初尘确认）**：上下文缓存与聚合桶持久化到本地状态文件；恢复只恢复最近 `groupCacheRecoveryMaxAgeMs`（默认 86400000 = 24h）内的条目。Gateway/桥接关闭超过恢复窗口后，旧缓存直接丢弃、不跨天恢复（"关一两天再开，该丢就丢"）。恢复窗口可通过 UI/API/env `QQBOT_PRO_GROUP_CACHE_RECOVERY_MAX_AGE_MS` 修改，默认 86400000，范围 0～604800000（0 = 不恢复任何旧缓存）。
- 上下文查询数据源 = 持久化上下文缓存（Gateway 重启后仍可查），不是 Gateway 内存队列；查询结果只发给模型、不落盘。
...
1. `GROUP_AT_MESSAGE_CREATE` 作为默认触发事件（classifyEvent 按 eventType + groupMessageMode 分流）。
2. 普通 `GROUP_MESSAGE_CREATE` 只进上下文环形缓存，不唤醒 AI、不落 Operit 对话；进缓存后从 Gateway 队列移除，避免队列膨胀。
...
5. 测试普通消息不会自己触发 AI。
6. **查询数据源 = 持久化上下文缓存**（Gateway 重启后仍可查）；查询结果只发给模型、不落 Operit 对话。
7. 查询条数：默认 before/after 各 5、单次最大 20（`groupContextBefore`/`groupContextAfter`/`groupContextLimit`，UI/API/env 可改）。
