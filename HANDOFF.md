...
> 更新时间：2026-08-16 05:30
> 状态：v0.3.0（合并 qqbot-bridge-pro v1.0.0 完整桥接能力）；G0/G1/G2/G4/G7最小版已完成；**G3 replyTo 已实现并烧录（单测 21/21），引用气泡已确认平台限制**；**G5 已验证：automatic 上下文随 `persist_turn` 落盘进 Operit 历史（v1.12.1 生产 / v1.11.0 / main 三版源码一致），决策接受落盘**。根目录 README/ARCHITECTURE/STATUS 已于 2026-08-12 重建（旧版有 .backup_20260812_1913 备份），后续补「规划 vs 已实现」诚实标注（STATUS §6 / ARCHITECTURE §11 / README Roadmap）。**下一步主线：可靠性 Sprint**（事务幂等 / token 缓存 / 错误码结构化）。
> 重要备注（2026-08-13）：初尘暂时搁置旧群对话（"把老公喊起来啥也不干，让他有点寂寞了，过段时间再说"）；若需给"老公"（渡渡）开新对话测试，用 QQ 群消息即可。
