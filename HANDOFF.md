...
> 更新时间：2026-08-16 05:12
> 状态：v0.3.0（合并 qqbot-bridge-pro v1.0.0 完整桥接能力）；G0/G1/G2/G4/G7最小版已完成；**G3 replyTo 已实现并烧录（单测 21/21），引用气泡已确认平台限制（群聊被动回复客户端不渲染引用卡片）**；根目录 README/ARCHITECTURE/STATUS 已于 2026-08-12 重建（旧版有 .backup_20260812_1913 备份），后续补「规划 vs 已实现」诚实标注（STATUS §6 / ARCHITECTURE §11 / README Roadmap）。**下一步主线：G5 Hook 探针**（验证 automatic 上下文附件是否落盘）。
