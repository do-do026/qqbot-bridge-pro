...
class QQBotGatewayService:
    def __init__(self, args):
        ...
        self.control_port = int(args.control_port or 32146)
    ...
    # qqbot-pro 增强：INTERACTION_CREATE 等事件根据 payload 的 scene 字段判断
    def infer_scene(self, event_type: str, payload=None) -> str:
        if event_type.startswith("C2C_") or event_type.startswith("FRIEND_"):
            return "c2c"
        if event_type.startswith("GROUP_"):
            return "group"
        # 互动事件等：从 payload.d.scene 读取（c2c / group / guild）
        if payload is not None:
            data = payload.get("d") if is_object(payload.get("d")) else {}
            scene = first_non_blank(data.get("scene"))
            if scene in ("c2c", "group", "guild"):
                return scene
            chat_type = data.get("chat_type")
            if chat_type in (1, 2):
                return "group" if int(chat_type) == 1 else "c2c"
        return "unknown"

    # qqbot-pro 增强：放开全部事件入队（含 INTERACTION_CREATE / SUBSCRIBE_MESSAGE_STATUS 等）
    def should_queue_event(self, event_type: str) -> bool:
        # 消息类事件
        if event_type.startswith("C2C_") or event_type.startswith("GROUP_") or event_type.startswith("FRIEND_"):
            return True
        # 互动与生命周期类事件（qqbot-pro 新增放开）
        interaction_like = {
            "INTERACTION_CREATE",
        ...
        return event_type in interaction_like
    ...
    def build_event(self, payload):
        ...
        event = {
            "scene": self.infer_scene(event_type, payload),
