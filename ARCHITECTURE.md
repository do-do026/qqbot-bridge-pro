...
> 状态：**已按本契约实现并烧录**（含 message_reference 引用，单测 21/21）。**已确认**：①引用气泡受平台限制（群聊被动回复客户端不渲染引用卡片，见 §7.5）；**待验证**：②active_send 主动群消息是否被平台接受 ③AI（角色卡）对 replyTo 协议的遵循率。以下为契约原文，实现细节以源码为准。
...
- `message_reference` 已接到 AI 选择的群消息锚点（`sendReplyToQQAsync` 群聊分支同时携带 `msg_id` 被动回复 + `message_reference` 引用）。
- **引用气泡平台限制（2026-08-16 实测确认）**：群聊机器人被动回复场景下，QQ 客户端收到 `msg_id`（被动回复）后按"回复"渲染，**不渲染 `message_reference` 的引用卡片**（平台侧已返回 `ref_idx`，但客户端不显示）。这是平台/客户端行为，非 bug。保留 `message_reference` 无害（频道或未来版本可能生效）；不强行去掉 `msg_id`——去掉会变成主动消息，失去被动回复时效保护与主动消息额度。
- 真正的客户端原生 @、主动群消息是否被平台接受，必须实机验证后才可写入文档为「已验证」。
...
| automatic 上下文会落盘 | 随轮次进入 Operit 历史（`persist_turn` 落盘，G5 已验证） | 接受现状；未来可用 Prompt Hook 隔离 |
| UI 未完成 | 配置只能走 API/env | ✅ G6 已完成（2026-08-16 compose_dsl 设置页） |
| src/dist 手工同步 | 漏同步会烧旧代码 | sync.sh 已缓解，长期建编译流程 |
| `target_chat_id` 兼容读取残留 | 误导新读者 | 文档已声明废弃，代码后续清理 |

## 11. 规划路线（尚未实现，勿误读为已做）

以下能力已在设计/规划中，**当前均未实现**。判断某个能力是否存在，以本清单和 STATUS §6 为准：

| 规划项 | 目标 | 当前状态 |
|---|---|---|
| G7 完整版 | UI 管理群成员绑定（取代裸配置 API） | 🔴 未实现 |
| 可靠性 Sprint | 事务级幂等、access_token 缓存、错误码/Trace ID 结构化 | 🔴 未实现 |
| 原生 @ / 引用样式 | 群内客户端原生 @ 与引用气泡的真机验收 | ⚪ 引用气泡已确认平台限制（群聊被动回复客户端不渲染）；原生 @ 待验证 |
| 双账号隔离实测 | 两个 C2C 用户互不串线 | 🔴 未实测（设计已完成） |
| 官方 stream_messages | 流式主动消息 | ❌ 产品决定不做（非规划） |

*本文件由渡渡与初尘维护，随工程迭代更新。当前主线入口：可靠性 Sprint（见 STATUS §6 / 本文件 §10）。已完成：G0-G6（G6 UI 2026-08-16）、G5 落盘验证、G3 replyTo。*