...
> 状态：**已按本契约实现并烧录**（2026-08-13，含 message_reference 引用修复，单测 21/21）。**待真机验证**：①引用气泡是否显示 ②active_send 主动群消息是否被平台接受 ③AI（角色卡）对 replyTo 协议的遵循率。以下为契约原文，实现细节以源码为准。
...
| 原生 @ / 引用样式 | 群内客户端原生 @ 与引用气泡的真机验收 | 🟡 部分（引用修复已烧录，待验证） |
